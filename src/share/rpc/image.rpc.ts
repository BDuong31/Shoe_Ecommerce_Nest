...
export class ImageRPCClient implements IPublicImageRpc {
    ...
    async getImagesByRefId(refId: string, type: string, isMain?: boolean): Promise<PublicImage[]> {
        try {
            if (isMain === undefined) {
                const { data } = await axios.post(`${this.favoriteServiceUrl}/images/rpc/get-by-ref-id`, { refId, type });
                return data.data;
            } else {
                const { data } = await axios.post(`${this.favoriteServiceUrl}/images/rpc/get-by-ref-id`, { refId, type, isMain });
                return data.data;
            }        
        } catch (error) {
            return [];
